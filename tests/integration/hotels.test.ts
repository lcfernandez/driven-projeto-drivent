...
import { createEnrollmentWithAddress, createHotel, createPayment, createTicket, createTicketType, createUser, generateCreditCardData } from "../factories";
...
describe("GET /hotels", () => {
    ...
    describe("when token is valid", () => {
        ...
        it("should respond with status 404 when ticket was paid, isnt remote, hotel is included but there are no hotels created", async () => {
            const user = await createUser();
            const token = await generateValidToken(user);
            const enrollment = await createEnrollmentWithAddress(user);
            const ticketType = await prisma.ticketType.create({
